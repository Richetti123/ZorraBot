...
if (command == 'play.1') {
  ...
  try { 
    const mediaa = await yta(text);
    const aa = await conn.sendMessage(m.chat, {audio: {url: mediaa.dlink }, fileName: `error.mp3`, mimetype: 'audio/mp4'}, {quoted: m});
    if (!aa) {
      throw new Error();
    }} catch {
    ...
    if (command == 'play.2') {
      ...
      try {
        const mediaa = await ytv(text);
        const aa_2 = await conn.sendMessage(m.chat, {video: {url: mediaa.dlink}, fileName: `error.mp4`, caption: `${wm}`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
